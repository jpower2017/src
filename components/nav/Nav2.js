...
import { stages, stage } from "../../common/config";
...
class Nav extends Component {
  constructor(props) {
    ...
    this.getNoTooltip = this.getNoTooltip.bind(this);
  }
  componentWillMount() {
  ...
  getObj(id) {
    console.log("getObj");
    console.table(R.find(R.propEq("id", id))(this.state.data));
  ...
  getArrayofAuthApps(data, filterObj) {
    ...
    let temp = null;
    console.log("stages, stage " + [stages.DEV, stage]);
    temp = stages.DEV == stage ? data : null;
    if (temp) return data;
  ...
  updateSiblings(id, level) {
    console.log("updateSiblings " + [id, level]);
    return R.map(
  ...
  onclick(item) {
    ...
    if (!isLeaf) {
      console.log("!isLeaf " + [id, level]);
      let b = this.updateSiblings(id, level);
      this.setState({ ["items" + level]: b });
      console.log("!isLeaf " + JSON.stringify(b));
  ...
  updateChildren(id, level) {
    console.log("updateChildren " + [id, level]);
    return R.map(
  ...
  getNoTooltip() {
    console.log("getNoTooltip");
    return (
      <div style={{ marginLeft: 30 }} onClick={() => this.addToFavs()}>
        <CircleAdd color="white" />
      </div>
    );
  }
