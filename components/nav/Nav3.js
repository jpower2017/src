...
class Nav extends Component {
  ...
  /* TEMPORARY ALL ON auth apps, by returning return */
  getArrayofAuthApps(data, filterObj) {
    console.log("getArrayofAuthApps f");
    console.table(data);
    console.log(JSON.stringify(filterObj));
    //const banks = R.prop("PositivePayBanks", bankData);
    const arr = R.prop("queries", filterObj);
    /* temp */
    //return data;
    /**/
    ...
    const f = (name, data) => R.filter(x => R.contains(x.allow, arr), data);
    console.log("herehere " + R.flatten(f));
    return;
    return R.flatten(f);
  }
  ...
  renderMainNavHeader() {
    console.log(
      "this.props.authorizations " + JSON.stringify(this.props.authorizations)
    );
    return (
      <div
        style={{
